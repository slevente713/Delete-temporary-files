...
```sh
./NoWinUpdateTempDeletion_DeleteTemporaryFiles5_1.ps1
```
...
- Windows Update junk files (C:\Windows\SoftwareDistribution\Download) [```NoWinUpdateTempDeletion_DeleteTemporaryFiles5_1.ps1``` doesn't remove these files]
- Delivery Optimization cache (C:\Windows\DeliveryOptimization\Cache) [```NoWinUpdateTempDeletion_DeleteTemporaryFiles5_1.ps1``` doesn't remove these files]
- Windows Error Reporting files (C:\Windows\Minidump) ($env:"USERPROFILE"\AppData\Local\CrashDumps)
- [Optional] Old, not used Windows components from WinSxS (DISM /Online /Cleanup-Image /StartComponentCleanup /ResetBase)
...
- [Optional] NVIDIA cache, Logs, and temporary files (C:\ProgramData\NVIDIA Corporation\Logs) (C:\ProgramData\NVIDIA Corporation\NVBackend) (C:\ProgramData\NVIDIA Corporation\Downloader) (C:\ProgramData\NVIDIA Corporation\NV_Cache) [The script will only delete fiels from these, that are older than 15 days.]
