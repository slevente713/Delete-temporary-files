...
$question3 = Read-Host "Szeretnéd kikapcsolni a SysMain (vagy SuperFetch) szolgáltatást hogy a Prefetch mappába ne generálódjon több fájl? (HDD-n nem javasolt) [Y/N] Would you like to turn off SysMain (or SuperFetch) to don't generate more files to the Prefetch folder? (Not recomended with HDDs.) [Y/N]"
...
$question4 = Read-Host "Szeretnéd törölni az előző Windows telepítésed maradványait (Windows.old)? [Y/N] Would you like to delete your previous Windows install's junk files (Windows.old)? [Y/N]"
if ($question4 -match "y") {
    $path4 = "C:\Windows.old"
    if (Test-Path $path4) {
        rm "$path4\*" -Recurse -Force -ErrorAction SilentlyContinue
