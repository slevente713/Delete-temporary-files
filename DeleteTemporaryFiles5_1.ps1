Write-Host "Welcome in Delete-Temporary-Files!" -ForegroundColor Blue
...
$isAdmin = ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)
    if ($isAdmin) {
        ...
        Write-Host "Removing Windows Update junk files..." -ForegroundColor Cyan
            rm "C:\Windows\SoftwareDistribution\Download\*" -Recurse -Force -ErrorAction SilentlyContinue
        Write-Host "Successfully removed Windows Update junk files." -ForegroundColor Green

        Write-Host "Removing Delivery Optimization files..." -ForegroundColor Cyan
            rm "C:\Windows\DeliveryOptimization\Cache\*" -Recurse -Force -ErrorAction SilentlyContinue
        Write-Host "Successfully removed Delivery Optimization files." -ForegroundColor Green
        ...
        $question5 = Read-Host "Would you like to remove, all, old, not used Windows components, from WinSxS? [y/N]"
            if ($question5 -match "y") {
                DISM /Online /Cleanup-Image /StartComponentCleanup /ResetBase
                Write-Host "Successfully removed all, old, not used Windows components, from WinSxS." -ForegroundColor Green
            }
            else {
                Write-Host "You cancelled removing all, old, not used Windows components, from WinSxS." -ForegroundColor Yellow
            }
        $question6 = Read-Host "Would you like to remove NVIDIA cache files, and NVIDIA temporary files? [WARNING]: ONLY ANSWER Y/y TO THIS QUESTION, IF YOU HAVE AN NVIDIA GPU! (Some games may load a little bit slower.) (It will only delete files, that older than 15 days.) [y/N]" 
