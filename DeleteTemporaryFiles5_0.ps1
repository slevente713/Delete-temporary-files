...
if (-not $isAdmin) {
    Write-Host "[WARNING]: You ran this script, without elevated priviliges. Some features may not work. If you can, please restart the scipt with elevated priviliges!" -ForegroundColor Red
...
Write-Host "Removing old, not used components from WinSxS..." -ForegroundColor Cyan
DISM /Online /Cleanup-Image /StartComponentCleanup /ResetBase
Write-Host "Successfully removed old, not used components from WinSxS." -ForegroundColor Green
...
if ($question2 -match "y") {
    ...
    Remove-Item $_.FullName -Force -ErrorAction SilentlyContinue
        ...
        rm "C:\Windows\Logs\StorGroupPolicy.log" -Force -ErrorAction SilentlyContinue
...
if ($question3 -match "y") {
        rm -r "C:\Windows.old" -Recurse -Force -ErrorAction SilentlyContinue
